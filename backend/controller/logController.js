...
// ── UPDATE A LOGGED PROBLEM (favorite / solution / notes) ─
export const updateLoggedProblem = async (req, res, next) => {
  try {
    const { date, titleSlug } = req.params;
    const { isFavorite, solution, notes } = req.body;
    ...
    const entry = log.problemsSolved.find((p) => p.titleSlug === titleSlug);
    if (!entry) {
      return res.status(404).json({
        success: false,
        error: "Problem not found in this log",
    ...
    if (typeof isFavorite === "boolean") entry.isFavorite = isFavorite;
    if (typeof solution === "string") entry.solution = solution;
...
// ── MARK A FAVORITED PROBLEM AS REVISED TODAY ─────────────
export const markRevised = async (req, res, next) => {
  try {
    const { date, titleSlug } = req.params;
    ...
    entry.lastRevisedAt = new Date();
    await log.save();

    res.status(200).json({ success: true, data: entry, message: "Marked as revised" });
  } catch (error) {
    next(error);
  }
};

// ── GET REVISION QUEUE (all favorited problems, oldest revised first) ─
export const getFavorites = async (req, res, next) => {
  try {
    const logs = await DailyLog.find({
      user: req.user._id,
      "problemsSolved.isFavorite": true,
    }).select("date problemsSolved");
    ...
    // Never-revised problems first, then oldest-revised first
    favorites.sort((a, b) => {
      if (!a.lastRevisedAt && !b.lastRevisedAt) return new Date(a.solvedAt) - new Date(b.solvedAt);
      if (!a.lastRevisedAt) return -1;
      if (!b.lastRevisedAt) return 1;
      return new Date(a.lastRevisedAt) - new Date(b.lastRevisedAt);
    });

    res.status(200).json({ success: true, data: favorites, count: favorites.length });
  } catch (error) {
    next(error);
  }
};
